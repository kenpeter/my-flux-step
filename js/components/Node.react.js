...
var Node = React.createClass({
  ...
  render: function() {
    ...
    var selected = this.props.selected;
		...
		//test
		var content_class = 'node-content';
		if (typeof selected == 'undefined') {
      content_class = 'node-content';
    } 
		else {
      if (selected == node.key) {
				content_class = 'node-content highlight';
      } 
			else {
				content_class = 'node-content';	
      }
    ...
    if (typeof node.children != 'undefined') {
      ...
      return (
        <li className="node-item">
          <SelectedMarker currNodeKey={node.key} selected={selected} />
          <div className={content_class} onClick={this._onClick}>{node.key} - {node.content}</div>
